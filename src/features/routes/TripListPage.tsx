      ...
      <ScrollArea className="flex-1">
        <div className="space-y-10 p-6">
          ...
          {inProgressTrips.length > 0 && (
            <TripSection title="W trakcie" trips={inProgressTrips} view={view} />
          )}
          {completedTrips.length > 0 && (
            <TripSection title="Zakończone" trips={completedTrips} view={view} />
          )}
        </div>
      </ScrollArea>
    </PageLayout>
...
function TripGridCard({ trip }: { trip: UserTrip }) {
  return (
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col p-2.5">
        ...
        <div className="mt-auto grid w-full shrink-0 grid-cols-3 gap-1.5 pt-1.5">
          ...
          <Button
            ...
            variant="outline"
            className="h-8 w-full justify-center gap-1 px-1.5"
            onClick={() => exportJournalTrip(trip)}
          >
            <Download className="size-3.5 shrink-0" />
            <span className="truncate">Eksportuj</span>
          </Button>
        </div>
      </CardContent>
    </Card>
...
function TripListRow({ trip }: { trip: UserTrip }) {
  const entryCount = trip.journalEntries.length
